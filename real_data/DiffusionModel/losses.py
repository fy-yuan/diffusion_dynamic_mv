...
import numpy as np
from DiffusionModel.util import get_score_fn
from DiffusionModel.sde_lib import VESDE, VPSDE
...
def get_sde_loss_fn(sde, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5):
    ...
    def loss_fn(model, batch, cond):
        ...
        loss = torch.mean(losses) #+ 0.01*(score**2).mean() #TBD here, regularization 
