...
def gen_td3(ac, num_assets, fea_dim, time_horizon, scenario_pool, 
         actor_critic=core.MLPActorCritic_TD3, seed=42, 
         steps_per_epoch=200, epochs=150, pool_size = 1000, replay_size=int(1e5),
         scale_t=1,
         regularization1=0.0, regularization2=0.0, regularization_time = 0,
         polyak=0.99, pi_lr=1e-3, q_lr=1e-3, pi_lr_min = 1e-6, q_lr_min = 1e-6, batch_size=16, start_steps=200, 
   ...
   #logger.save_config(locals())
    ...
    def compute_loss_q(experience, scenario, time):
        ...
        #multiplier = multiplier.unsqueeze(1).expand(batch_size,-1) # Random multiplier
        obs = torch.cat([feature[time], wealth, multiplier], dim=-1)
        #expanded_action = action.unsqueeze(1).expand(batch_size,-1) # Expand action to match multiplier size
        #expanded_obs = torch.cat([obs, multiplier], dim=-1)
        #expanded_wealth = wealth.unsqueeze(1).expand(batch_size,-1) # Expand wealth to match multiplier size


        q1 = ac.q1(time, obs, action, scale_t)
        q2 = ac.q2(time, obs, action, scale_t)
        ...
        if time <= T-2:
           ...
           obs_next = torch.cat([feature[time+1], wealth_next, multiplier], dim=-1)
           #expanded_obs_next = torch.cat([obs_next.unsqueeze(1).expand(-1,batch_size,-1), multiplier], dim=-1)

           # Bellman backup for Q function
           with torch.no_grad():
               q1_pi_targ = ac_targ.q1(time+1, obs_next, ac_targ.pi(time+1, obs_next, scale_t),scale_t)
               q2_pi_targ = ac_targ.q2(time+1, obs_next, ac_targ.pi(time+1, obs_next, scale_t),scale_t)
        
        else:
           with torch.no_grad():
               q1_pi_targ = (wealth - multiplier).squeeze(-1)
               q1_pi_targ = q1_pi_targ**2 
               q2_pi_targ = (wealth - multiplier).squeeze(-1)
    ...
    def compute_loss_pi(experience, scenario, time,steps):
        ...
        #multiplier = multiplier.unsqueeze(0).expand(batch_size,-1,-1) 
        obs = torch.cat([feature[time], wealth, multiplier], dim=-1)
        #expanded_obs = torch.cat([obs.unsqueeze(1).expand(-1,batch_size,-1), multiplier], dim=-1)
        action = ac.pi(time, obs, scale_t)
        #q_pi = ac.q1(time, expanded_obs, action) + 0.05*torch.sum(action * action, dim=-1, keepdim=True)
        if steps <= epochs*steps_per_epoch*regularization_time:
           q_pi = ac.q1(time, obs, action, scale_t) + regularization1*torch.sum(action * action, dim=-1, keepdim=True)
        else:
           q_pi = ac.q1(time, obs, action, scale_t) + regularization2*torch.sum(action * action, dim=-1, keepdim=True)
        return q_pi.mean()
    ...
    pi_scheduler = CosineAnnealingLR(pi_optimizer, T_max=total_steps, eta_min=pi_lr_min)
    q_scheduler = CosineAnnealingLR(q_optimizer, T_max=total_steps, eta_min=q_lr_min)
    ...
    def update(experience, scenario, time, update_flag,steps):
        ...
        if (update_flag % policy_delay == 0) and (time<= T-2):
    ...
    def get_action(time, obs, noise_scale):
        a = ac.act(time, obs, scale_t)  # output action
        baseline = 0.01
    ...
    for steps in range(total_steps):
      ...
      scenario = scenario_pool.sample_batch(return_idx = True, batch_size = 1)
      price, feature, idx = scenario['price'], scenario['feature'], scenario['scenario_idx'].cpu().numpy()
    ...
    # The loop for time_horizon
      for t in range(T):
        ...
        replay_buffer.store(wealth, a, multiplier, idx)  # Store the scenario index for later use
        if t <= T-2:
           a = wealth * a / price[t]
        ...
        if steps >= update_after and (steps+1) % update_every == 0:
            for j in range(update_every):
                ...
                scenario_batch = scenario_pool.get_scenario(experience_batch['scenario_idx'].cpu().numpy())
